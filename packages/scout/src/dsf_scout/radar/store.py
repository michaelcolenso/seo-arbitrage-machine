...
class RadarStore:
        ...
        *,
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                """,
                (
                    run_id, name, ScanStatus.PENDING.value, total_expected,
                    json.dumps(config or {}, sort_keys=True), source_snapshot, now, now,
                ),
            )
    ...
    def save_batch(
        ...
        *,
        ...
        inserted = promoted = review = rejected = 0
        with self._connect() as conn:
            for result in rows:
                c = result.candidate
                cursor = conn.execute(
                    ...
                    """,
                    (
                        run_id, c.keyword, c.source, c.volume, c.cpc, c.kd, c.intent, c.buyer,
                        c.decision_value, c.monetization_ease, c.defensibility, c.complexity,
                        c.data_leverage, c.evidence_quality, result.scan_score,
                        result.business_score, result.opportunity_score, result.decision.value,
                        json.dumps(result.reasons), c.model_dump_json(), result.scored_at.isoformat(),
                    ),
                )
            ...
            conn.execute(
                ...
                """,
                (
                    inserted, promoted, review, rejected, spend_delta_usd,
    ...
    def get_summary(self, run_id: str | None = None) -> ScanSummary | None:
        ...
        with self._connect() as conn:
            row = (
                conn.execute("SELECT * FROM radar_scan_runs WHERE run_id = ?", (run_id,)).fetchone()
                if run_id
                else conn.execute(
        ...
        if row is None:
            return None
        return ScanSummary(
            run_id=row["run_id"], name=row["name"], status=ScanStatus(row["status"]),
            total_expected=row["total_expected"], processed_count=row["processed_count"],
            promoted_count=row["promoted_count"], review_count=row["review_count"],
            rejected_count=row["rejected_count"], error_count=row["error_count"],
            spend_usd=row["spend_usd"], checkpoint=row["checkpoint"],
